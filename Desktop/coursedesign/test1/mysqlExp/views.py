...
def searchhistory(request):
    ...
    sql = "select id from mysqlexp_user where username='{}'".format(username)
    try:
        cursor.execute(sql)
        res = cursor.fetchall()
        uid = res[0][0]
...
def login(request):
    ...
    cursor = conn.cursor()
    try:
        sql = "select username from mysqlexp_user where username='{}'".format(username)
        cursor.execute(sql)
        res = cursor.fetchall()
    except:
        return HttpResponse(None)
    ...
    if (len(res) == 0):
        ...
        try:
            sql = "select password from mysqlexp_user where username='{}'".format(username)
            cursor.execute(sql)
            res = cursor.fetchall()
            res = str(res[0][0])
            if res == password:
...
def register(request):
    ...
    sql = "insert into mysqlexp_user (username,password) values ('{}','{}')".format(username, password)
    try:
...
def updatepassword(request):
    ...
    sql = "update mysqlexp_user set password='{}' where username='{}'".format(password, username)
    try:
        flag = cursor.execute(sql)
        conn.commit()
    except:
